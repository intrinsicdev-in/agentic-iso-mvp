...
const aiSuggestionService = new ai_suggestion_service_1.AISuggestionService();
// Get all AI suggestions with optional filtering
router.get('/', async (req, res) => {
    try {
        const { status, type, agentType, agentId } = req.query;
        const filters = {};
        if (status) {
...
router.post('/agents', async (req, res) => {
    try {
        ...
        if (!Object.values(client_1.AgentType).includes(type)) {
            return res.status(400).json({
...
router.post('/', async (req, res) => {
    try {
        const { agentId, type, title, content, rationale, metadata } = req.body;

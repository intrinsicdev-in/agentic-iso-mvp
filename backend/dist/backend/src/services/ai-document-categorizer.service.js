...
exports.AIDocumentCategorizerService = void 0;
const openai_1 = require("openai");
const client_1 = require("@prisma/client");
const prisma = new client_1.PrismaClient();
// Initialize OpenAI if API key is available
const openai = process.env.OPENAI_API_KEY ? new openai_1.OpenAI({
    apiKey: process.env.OPENAI_API_KEY,
}) : null;
class AIDocumentCategorizerService {
    /**
     * Use AI to analyze document content and suggest ISO standard mappings
     */
    async categorizeDocument(documentTitle, documentContent, standard) {
        if (!openai) {
        ...
        const prompt = this.buildCategorizationPrompt(documentTitle, standardDocsContext, documentContent);
        try {
     ...
     */
    buildCategorizationPrompt(documentTitle, standardDocs, documentContent) {
        return `
...
`;
     ...
     */
    validateAndEnhanceResult(aiResult, standardDocs) {
        ...
        if (aiResult.suggestedMappings && Array.isArray(aiResult.suggestedMappings)) {
            for (const mapping of aiResult.suggestedMappings) {
                ...
                const standardDoc = standardDocs.find(doc => doc.title === mapping.standardDocument);
     ...
     */
    fallbackCategorization(documentTitle, documentContent, standardDocs) {
     ...
     */
    async batchCategorizeDocuments(documentIds) {
     ...
     */
    async autoApplyMappings(documentId, categoryResult, userId, minConfidence = 0.8) {
        let appliedCount = 0;
...
exports.AIDocumentCategorizerService = AIDocumentCategorizerService;
//# sourceMappingURL=ai-document-categorizer.service.js.map
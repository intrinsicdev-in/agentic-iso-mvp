...
router.post('/create-reference', auth_1.authenticateToken, async (req, res) => {
    try {
...
router.post('/set-parent', auth_1.authenticateToken, async (req, res) => {
    try {
...
router.get('/:documentId/relationships', auth_1.authenticateToken, async (req, res) => {
    try {
...
router.post('/discover-relationships', auth_1.authenticateToken, async (req, res) => {
    try {
...
async function checkCircularReference(childId, parentId) {
    const checkParent = async (currentParentId) => {
...
function analyzeDocumentRelationship(doc1, doc2) {
    const title1 = doc1.title.toLowerCase();
    const title2 = doc2.title.toLowerCase();
    // Manual/Policy relationship
    if (doc1.documentType === client_1.DocumentType.MANUAL &&
        doc2.documentType === client_1.DocumentType.POLICY) {
        if (title1.includes('manual') && (title2.includes('policy') || title2.includes('objective'))) {
            return {
                type: client_1.ReferenceType.IMPLEMENTS,
                confidence: 0.8,
    ...
    if (doc1.documentType === client_1.DocumentType.PROCEDURE &&
        doc2.documentType === client_1.DocumentType.POLICY) {
        return {
            type: client_1.ReferenceType.IMPLEMENTS,
            confidence: 0.7,
    ...
    if (doc1.documentType === client_1.DocumentType.LOG &&
        doc2.documentType === client_1.DocumentType.PLAN) {
        return {
            type: client_1.ReferenceType.SUPPORTS,
            confidence: 0.6,
    ...
    if (commonWords.length > 0) {
        ...
        if (similarity > 0.3) {
            return {
                type: client_1.ReferenceType.CROSS_REFERENCE,
...
//# sourceMappingURL=document-relationships.js.map
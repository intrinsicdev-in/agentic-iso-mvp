...
router.get('/', auth_1.authenticateToken, (0, auth_1.requireRole)(['SUPER_ADMIN', 'ACCOUNT_ADMIN']), async (req, res) => {
    try {
        const { page = '1', limit = '50', search = '', entityType = '', action = '', userId = '', dateRange = '7' } = req.query;
...
router.get('/:id', auth_1.authenticateToken, (0, auth_1.requireRole)(['SUPER_ADMIN', 'ACCOUNT_ADMIN']), async (req, res) => {
    try {
        const { id } = req.params;
...
// Export audit logs
router.get('/export/csv', auth_1.authenticateToken, (0, auth_1.requireRole)(['SUPER_ADMIN', 'ACCOUNT_ADMIN']), async (req, res) => {
    try {
        const { entityType = '', action = '', dateRange = '30' } = req.query;

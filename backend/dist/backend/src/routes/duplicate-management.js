...
router.get('/detect', auth_1.authenticateToken, async (req, res) => {
    try {
...
router.get('/group/:baseDocument', auth_1.authenticateToken, async (req, res) => {
    try {
        const organizationId = req.user?.organizationId;
        ...
        const detector = new duplicate_detector_1.DuplicateDetector();
        const result = await detector.detectDuplicates(organizationId);
        const group = result.duplicateGroups.find(g => g.baseDocument === decodeURIComponent(baseDocument));
        if (!group) {
        ...
        const detailedDocuments = await Promise.all(group.documents.map(async (doc) => {
...
router.post('/merge', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        // Perform the merge
        const result = await (0, duplicate_detector_1.mergeDuplicates)(documentIds, keepDocumentId, userId);
        res.json({
            success: true,
            data: result
        });
...
router.delete('/delete/:documentId', auth_1.authenticateToken, async (req, res) => {
    try {
        const organizationId = req.user?.organizationId;
        const userId = req.user?.id;
...
router.post('/bulk-resolve', auth_1.authenticateToken, async (req, res) => {
    try {
        const organizationId = req.user?.organizationId;
        const userId = req.user?.id;
        ...
        for (const resolution of resolutions) {
            ...
            try {
                if (action === 'merge' && documentIds && keepDocumentId) {
                    const mergeResult = await (0, duplicate_detector_1.mergeDuplicates)(documentIds, keepDocumentId, userId);
                    results.push({
                        action: 'merge',
                        success: true,
                        result: mergeResult
...
//# sourceMappingURL=duplicate-management.js.map
...
class DuplicateDetector {
    constructor() {
        this.matcher = new document_matcher_1.DocumentMatcher();
    }
    /**
     * Detect duplicate documents within an organization
     */
    async detectDuplicates(organizationId) {
        ...
        for (let i = 0; i < documents.length; i++) {
            if (processedIds.has(documents[i].id))
                continue;
            ...
            for (let j = i + 1; j < documents.length; j++) {
                if (processedIds.has(documents[j].id))
                    continue;
    ...
    calculateDocumentSimilarity(doc1, doc2) {
        ...
        const clauses1 = new Set(doc1.clauseMappings.map((cm) => cm.clause.id));
        const clauses2 = new Set(doc2.clauseMappings.map((cm) => cm.clause.id));
     ...
     */
    determineRecommendedAction(group) {
 ...
 */
async function mergeDuplicates(documentIds, keepDocumentId, userId) {
    try {
        // Get all documents
        const documents = await prisma.artefact.findMany({
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            const existingClauseMappings = new Set(keepDocument.clauseMappings.map(cm => cm.clauseId));

...
exports.findMissingDocuments = findMissingDocuments;
...
class DocumentMatcher {
     ...
     */
    async matchDocument(existingDoc, standardDoc) {
     ...
     */
    checkClauseMatch(existingDoc, standardDoc) {
        ...
        const exactMatches = standardDoc.clauseNumbers.filter(clause => existingClauses.includes(clause));
        ...
        const partialMatches = standardDoc.clauseNumbers.filter(stdClause => existingClauses.some(existingClause => stdClause.startsWith(existingClause) || existingClause.startsWith(stdClause)));
     ...
     */
    checkTitleMatch(existingTitle, standardTitle) {
     ...
     * Check for abbreviation matches
     */
    checkAbbreviationMatch(existingTitle, standardTitle) {
     ...
     */
    checkKeywordMatch(existingTitle, keywords) {
        const normalizedTitle = existingTitle.toLowerCase();
        const matchedKeywords = keywords.filter(keyword => normalizedTitle.includes(keyword.toLowerCase()));
 ...
 */
async function findMissingDocuments(organizationId, standard) {
...
//# sourceMappingURL=document-matcher.js.map
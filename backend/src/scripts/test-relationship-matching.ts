...
async function testRelationshipMatching() {
  ...
  try {
    ...
    const missingBefore = await findMissingDocumentsWithRelationships(testOrg.id);
    ...
    const missingAfter = await findMissingDocumentsWithRelationships(testOrg.id);
    ...
    const iso9001Missing = await findMissingDocumentsWithRelationships(testOrg.id, StandardType.ISO_9001_2015);
    console.log(`  📊 ISO 9001 missing documents: ${iso9001Missing.length}`);
    
    console.log('\n🔍 Test Scenario 7: ISO 27001 specific check');
    
    const iso27001Missing = await findMissingDocumentsWithRelationships(testOrg.id, StandardType.ISO_27001_2022);

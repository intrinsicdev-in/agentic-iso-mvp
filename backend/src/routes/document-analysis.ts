...
import { AIDocumentCategorizerService } from '../services/ai-document-categorizer.service';
...
router.post('/analyze-documents', authenticateToken, async (req, res) => {
  try {
    ...
    const standardWhereClause = standard 
      ? { standard: standard as StandardType }
      : { 
          OR: [
...
router.post('/apply-mappings', authenticateToken, async (req, res) => {
  try {
    const organizationId = req.user?.organizationId;
...
router.get('/fulfillment-status', authenticateToken, async (req, res) => {
  try {
    const organizationId = req.user?.organizationId;
...
router.post('/ai-categorize', authenticateToken, async (req, res) => {
  try {
    const { documentIds, standard } = req.body;
    const organizationId = req.user?.organizationId;
    ...
    const aiCategorizer = new AIDocumentCategorizerService();
...
router.post('/ai-auto-apply', authenticateToken, async (req, res) => {
  try {
    ...
    const appliedCount = await aiCategorizer.autoApplyMappings(
      documentId,
      categoryResult,
      userId,
      minConfidence
